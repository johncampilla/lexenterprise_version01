...
def main(request):
    ...
    matterlist = Matters.objects.filter(created_at__year = today.year, created_at__month = today.month, handling_lawyer__lawyerID__userid=access_code).order_by('-filing_date')
    duedates = AppDueDate.objects.filter(duedate__year = today.year, duedate__month = today.month, matter__handling_lawyer__lawyerID__userid=access_code).order_by('duedate')
    recentdocs = FilingDocs.objects.filter(Task_Detail__lawyer__lawyerID__userid=access_code).order_by('-DocDate')
    countalert = alertmessages.count()
    multiple_q = Q(matter__handling_lawyer__lawyerID__userid=access_code)

    recenttask = task_detail.objects.filter(multiple_q, tran_date__year = today.year, tran_date__month = today.month).order_by('-tran_date')
...
def matterlist(request):
    ...
    if 'q' in request.GET:
        q = request.GET['q']
        multiple_q = Q(Q(matter_title__icontains=q) | Q(folder__client__client_name__icontains=q) | Q(folder__folder_description__icontains=q) | Q(referenceno__icontains=q)) 
        matters = Matters.objects.filter(multiple_q, handling_lawyer__lawyerID__userid=access_code).order_by("-filing_date")
...
def alert_messages(request):
    access_code = request.user.user_profile.userid
    messages = Alert_Messages.objects.filter(messageto=access_code).order_by('-date_alert')
...
def view_sentmessages(request):
    access_code = request.user.user_profile.userid
    sentmessages = Alert_Messages.objects.filter(sentby__userid=access_code).order_by('-date_alert')
    context = {
...
def view_portfolio(request, m_id):
    ...
    bill_amount = AccountsReceivable.objects.filter(matter__id=m_id).aggregate(Sum('bill_amount'))
    if bill_amount["bill_amount__sum"] == None:
        Total_bill_amt = 0
    else:
        Total_bill_amt = bill_amount["bill_amount__sum"]

    pay_amount = Payments.objects.filter(matter__id=m_id).aggregate(Sum('pay_amount'))
    ...
    task_summary = task_detail.objects.values('task_code__Activity').annotate(task_count=Count('task_code')).filter(matter__id=m_id)
...
def matter_review(request, pk):
    def validateduedates():
        def computeduedate():
            ...
            dues = AppDueDate.objects.filter(matter_id = matter_id, duedate=duedate)
            if dues.exists():
                pass
            else:
                duedates = AppDueDate(matter_id = matter_id, duedate=duedate, particulars=particulars)
                duedates.save()            
...
#            if apptype == "Trademark":
    ...
    bill_amount = AccountsReceivable.objects.filter(matter__id=pk).aggregate(Sum('bill_amount'))
    if bill_amount["bill_amount__sum"] == None:
        Total_bill_amt = 0
    else:
        Total_bill_amt = bill_amount["bill_amount__sum"]

    pay_amount = Payments.objects.filter(bill_number__matter__id=pk).aggregate(Sum('pay_amount'))
    if pay_amount["pay_amount__sum"] == None:
        Total_pay_amt = 0
    else:
        Total_pay_amt = pay_amount["pay_amount__sum"]
    ...
    task_summary = task_detail.objects.values('task_code__Activity').annotate(task_count=Count('task_code')).filter(matter__id=pk)
...
def billing_list(request):
    ...
    if 'q' in request.GET:
        q = request.GET['q']
        multiple_q = Q(Q(matter__matter_title__icontains=q) | Q(matter__folder__client__client_name__icontains=q) | Q(matter__folder__folder_description__icontains=q) | Q(bill_number__icontains=q) | Q(matter__referenceno__icontains=q)) 
        arlist = AccountsReceivable.objects.filter(multiple_q, matter__handling_lawyer__lawyerID__userid=access_code).order_by("-bill_date") 
    else: 
        #matters = Matters.objects.filter(handling_lawyer__lawyerID__userid=access_code).order_by("-filing_date")
        arlist = AccountsReceivable.objects.filter(matter__handling_lawyer__lawyerID__userid=access_code).order_by("-bill_date")
...
#    docs = MailsIn.objects.filter(Task_Detail__matter__handling_lawyer__lawyerID__userid=access_code and Task_Detail.doc_type = "").order_by('-DocDate')
    docs = task_detail.objects.filter(matter__handling_lawyer__lawyerID__userid=access_code, doc_type = 'Incoming').order_by('-tran_date')
...
def recentviewduedates(request, pk):
    ...
    matter = Matters.objects.get(id = matterid)
    ARBills = AccountsReceivable.objects.filter(matter__id = matterid, payment_tag = "UN")
    Total_bill_amount = AccountsReceivable.objects.filter(matter__id = matterid, payment_tag = "UN").aggregate(Sum('bill_amount'))
...
def myfolderlist(request):
    access_code = request.user.user_profile.userid
    username = request.user.username 
    result = Matters.objects.values('folder__folder_type__id','folder__folder_type__folder').annotate(NoOfMatters=Count('matter_title')).filter(handling_lawyer__lawyerID__userid=access_code).order_by('-NoOfMatters')
    print(result)
...
def myclientlist(request):
    access_code = request.user.user_profile.userid
    username = request.user.username 
    result = Matters.objects.values('folder__client__id','folder__client__client_name').annotate(NoOfMatter=Count('matter_title')).filter(handling_lawyer__lawyerID__userid=access_code).order_by('-NoOfMatter')
...
def myfolderdetail(request, pk):
    ...
    matters = Matters.objects.filter(folder__folder_type__id = pk, handling_lawyer__lawyerID__userid=access_code)
...
def myclientdetail(request, pk):
    ...
    matters = Matters.objects.filter(folder__client_id=pk, handling_lawyer__lawyerID__userid=access_code)
...
#    result = AccountsReceivable.objects.filter(, payment_tag='UNPAID')
    result = AccountsReceivable.objects.filter(lawyer__lawyerID__userid=access_code, payment_tag ='UN').order_by('matter')

    print(result)
    Unpaid =AccountsReceivable.objects.filter(lawyer__lawyerID__userid=access_code, payment_tag='UN').aggregate(Sum('bill_amount'))
...
def mybillingdetail(request, c_id, m_id, ar_id):
    ...
    payamt = Payments.objects.filter(bill_number = ar_id).aggregate(Sum('pay_amount'))
