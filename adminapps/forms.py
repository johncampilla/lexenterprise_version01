...
class TaskEntryForm1(forms.Form):
    ...
    #         'tran_date':NumberInput(attrs={'type':'date'}),
        ...
        matter = forms.ModelChoiceField(label="Matter", queryset=Matters.objects.all().order_by('matter_title'))
        tran_date = forms.DateField(label='Activity Date', widget=NumberInput(attrs={'type':'date'}))
        task_code = forms.ModelChoiceField(label='Task Code', queryset=ActivityCodes.objects.all())
        doc_type = forms.ChoiceField(label='Document Type', widget=forms.RadioSelect, choices=DOCTYPE)
        tran_type = forms.ChoiceField(label='Transaction Type', widget=forms.RadioSelect, choices = TRANTYPE)
        preparedby = forms.ModelChoiceField(label='Prepared By', queryset=User_Profile.objects.all())
        lawyer = forms.ModelChoiceField(label='Lawyer', queryset=Lawyer_Data.objects.all())
        task = forms.CharField(label="Activity ", widget=forms.Textarea(attrs={'rows':3}))
...
class PaymentEntryForm(forms.Form):
    ...
    matter = forms.ModelChoiceField(label="Matter", queryset=Matters.objects.all().order_by('matter_title'))
    bill_number = forms.ModelChoiceField(label="Bill Number", queryset=AccountsReceivable.objects.all().order_by('bill_date'))
    payment_date = forms.DateField(label='Payment Date', widget=NumberInput(attrs={'type':'date'}))
    ...
    or_date = forms.DateField(label='OR Date', widget=NumberInput(attrs={'type':'date'}))
    or_amount = forms.DecimalField(label="OR Amount")
    bank_details = forms.CharField(label="Bank Details", max_length=150)
    payment_reference = forms.CharField(label="Payment Reference", max_length=30)
    reference_date = forms.DateField(label='Reference Date', widget=NumberInput(attrs={'type':'date'}))
    Charge_details = forms.CharField(label="Charger Details if any", max_length=30)
...
class BillEntryForm(forms.Form):
    ...
    bill_date = forms.DateField(label='Bill Date', widget=NumberInput(attrs={'type':'date'}))
    currency = forms.ChoiceField(label='Currency', widget=forms.RadioSelect, choices=CURRENCY)
